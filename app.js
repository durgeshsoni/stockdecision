// ===== AI Stock Analyzer Pro - India Focused Edition =====
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Mode toggle
    document.getElementById('btnBeginner').addEventListener('click', () => switchMode('beginner'));
    document.getElementById('btnPro').addEventListener('click', () => switchMode('pro'));
...
function switchMode(mode) {
    currentMode = mode;
    document.getElementById('btnBeginner').classList.toggle('active', mode === 'beginner');
    document.getElementById('btnPro').classList.toggle('active', mode === 'pro');

    // If results are showing, switch the view
    if (lastAnalysis) {
...
function handleSearchInput() {
    ...
    dropdown.innerHTML = matches.map(s => `
        <div class="suggestion-item" data-symbol="${s.symbol}">
            <div><span class="sym">${s.symbol}</span> <span class="name">${s.name}</span></div>
...
async function apiGet(endpoint, params) {
    ...
    if (res.status === 429) throw new Error('Yahoo Finance rate limit hit. Please wait 1-2 minutes and try again.');
...
async function loadWorldMarkets() {
    ...
    const indices = [
        // Indian markets first
        { symbol: '^NSEI', name: 'Nifty 50', flag: '🇮🇳' },
        { symbol: '^BSESN', name: 'Sensex', flag: '🇮🇳' },
        { symbol: 'NIFTYMIDCAP150.NS', name: 'Nifty Midcap', flag: '🇮🇳' },
...
async function startAnalysis() {
    ...
    try {
        ...
        setTimeout(() => {
            ...
            document.getElementById('beginnerContent').classList.toggle('hidden', currentMode !== 'beginner');
            document.getElementById('mainContent').classList.toggle('hidden', currentMode !== 'pro');
        }, 300);
...
function renderResults(symbol, prices, summary, meta, analysis, cs, currency, insights) {
    ...
    document.getElementById('verdictText').textContent = `AI: ${analysis.action} | Tech: ${analysis.techScore}/100 | Fundamental: ${analysis.fundScore}/100 | Combined: ${analysis.score}/100`;
